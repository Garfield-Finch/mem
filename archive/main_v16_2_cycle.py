...
def train(epoch, loader, dic_model, scheduler, device):
    ...
    for i, (img_s, pose_s, img_t, pose_t) in enumerate(loader):
        ...
        transfer_input = (img_s_quant_t, img_s_quant_b, transfer_quant_t, transfer_quant_b)
        img_transfer_out = model_img(transfer_input, mode='TRANSFER')

        # cycle part
        transfer_cycle_quant_t, transfer_cycle_quant_b = model_transfer(pose_t_quant_t, pose_s_quant_t, img_t_quant_t,
                                                                        pose_t_quant_b, pose_s_quant_b, img_t_quant_b)
        transfer_cycle_input = (img_t_quant_t, img_t_quant_b, transfer_cycle_quant_t, transfer_cycle_quant_b)
        img_transfer_cycle_out = model_img(transfer_cycle_input, mode='TRANSFER')
        ...
        loss_cycle_quant_recon_t = criterion(transfer_cycle_quant_t, img_s_quant_t.clone().detach())
        loss_cycle_quant_recon_b = criterion(transfer_cycle_quant_b, img_s_quant_b.clone().detach())
        loss_cycle_quant_recon = loss_cycle_quant_recon_t + loss_cycle_quant_recon_b

        # loss_image_recon
        loss_image_recon = criterion(img_transfer_out, img_t) + criterion(img_s_out, img_s)
        loss_cycle_image_recon = criterion(img_transfer_cycle_out, img_s)
        ...
        loss = weight_loss_recon * (weight_loss_quant_recon * loss_quant_recon + loss_image_recon +
                                    weight_loss_quant_recon * loss_cycle_quant_recon + loss_cycle_image_recon)
