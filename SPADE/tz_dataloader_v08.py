...
def _gen_bbox(face_kpts):
    """
    :return: bbox, list containing for coordinates
    """
    face_xs = face_kpts[:, 0]
    face_ys = face_kpts[:, 1]
    x_min = face_xs.min()
...
class iPERDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_root, subset, transform=None):
        ...
        for i in self.lst_dir:
            ...
            sample_num = len(dir_img) // 7
            lst_img = dir_img[:1]
            for j in range(7):
                lst_img += dir_img[j * sample_num + 1:j * sample_num + 2]

            if self.subset == 'train':
                dir_img = lst_img
            elif self.subset == 'val':
                dir_img = lst_img + dir_img
            elif self.subset == 'test':
                dir_img = lst_img + dir_img[divide_point_test:]
            else:
    ...
    def __getitem__(self, item):
        ...
        # deal with face data
        nm_file_face = os.path.join('/'.join(img_pose_nm.split("/")[:-1]), 'openpose_outs.pkl') \
        ...
        lst_tsr_pose = []
        for i in range(8):
            img_app_nm = self.input_nm_list[index_s + i]
            # img_pose_nm = img_app_nm.replace('images_HD', 'skeletons_hand').replace('.jpg', '.jpg')
            img_pose_nm = _gen_pose_name(img_app_nm)
            img_app = Image.open(img_app_nm)
            img_pose = Image.open(img_pose_nm)
            ...
            lst_tsr_app.append(tsr_app_s.numpy())
            lst_tsr_pose.append(tsr_pose_s.numpy())
        lst_tsr_app = torch.from_numpy(np.array(lst_tsr_app))
        lst_tsr_pose = torch.from_numpy(np.array(lst_tsr_pose))

        return lst_tsr_app, lst_tsr_pose, tsr_app_t, tsr_pose_t, bbox
