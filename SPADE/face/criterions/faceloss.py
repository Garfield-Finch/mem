...
class FaceLoss(nn.Module):
    ...
    def __init__(self, pretrained_path='asset/spretrains/sphere20a_20171020.pth', factor=1):
        ...
        # from utils.visualizers import VisdomVisualizer
    ...
    def crop_head_bbox(self, imgs, bboxs):
        ...
        for i in range(bs):
            ...
            min_x = min(bboxs[i][0], bboxs[i][1])
            max_x = max(bboxs[i][0], bboxs[i][1])

            min_y = min(bboxs[i][2], bboxs[i][3])
            max_y = max(bboxs[i][2], bboxs[i][3])
