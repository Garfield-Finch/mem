...
def train(epoch, loader, model, optimizer, scheduler, device):
    ...
    for i, (img, label) in enumerate(loader):
        ...
        if i % 100 == 0:
            ...
            img_save_name = f'sample/app_v03/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.png'
            utils.save_image(
        ...
        if i % 200 == 0:
            torch.save(model.state_dict(), f'checkpoint/app_v03/vqvae_{str(epoch + 1).zfill(3)}.pt')
...
if __name__ == '__main__':
    ...
    loader, _, _ = iPERLoader(data_root=args.path, batch=args.batch_size, transform=transform).data_load()

    model = appVQVAE().to(device)
    ...
    for i in range(args.epoch):
        ...
        torch.save(model.state_dict(), f'checkpoint/app_v03/vqvae_{str(i + 1).zfill(3)}.pt')
